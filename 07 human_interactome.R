...
human_interactome = read.table(file = "07 human interactome/SD1.txt", header = T)
...
save(human_interactome, uni_proteins, uni_gene_symbol, file = "07 human interactome/human_interactome.RData")
...
save(ppi_network, ppi_v, ppi_e, file = "07 human interactome/interactome_igraph.RData")
...
load("06 master regulator identification/key_regulators.RData")
## conver mouse to human entrezID
kregulator_m2h = getLDS(attributes = c("entrezgene", "mgi_symbol"), 
                        filters = "entrezgene", 
                        values = key_regulators$entrezgene, 
                        mart = mouse, 
...
kregulator_interome0 = kregulator_m2h[kregulator_m2h$NCBI.gene.ID.1 %in% ppi_v$name, ]
kregulator_interome = merge(kregulator_interome0, key_regulators, by.x = "NCBI.gene.ID", by.y = "entrezgene")
...
save(kregulator_interome, kregulator_interome_list, file = "07 human interactome/kregulator_interome.RData")

# 2. established epilepsy genes 
epilp_genes0 = read.csv("07 human interactome/epilepsy genes_84.csv")
## get thier entrezID
load("03 brain cell type enrichment/human_mouse_biomaRt.RData")
epilp_genes_entrezID = getBM(attributes = c("hgnc_symbol", "entrezgene"), 
                             filters = "hgnc_symbol", 
...
# 3. targets of AEDs
AED_targets0 = read.csv("07 human interactome/AED_targets.csv")
...
AED_targets_entrezID = AED_targets_entrezID[-72, ] # delete a wrong match
AED_targets = merge(AED_targets, AED_targets_entrezID, by.x = "Target_ID", by.y = "UNIPROT")
...
AED_target_net = read.csv("07 human interactome/epilepsy_D_T_net.csv")
AED_target_net = merge(AED_target_net, AED_targets_interome, by = "Target_ID")
AED_target_net_list = split(AED_target_net$ENTREZID, AED_target_net$Drug_Name)

save(AED_targets, AED_targets_interome, AED_target_net, AED_target_net_list, file = "07 human interactome/AED_targets.RData")
...
closest_dis(epilp_genes_interome$entrezgene, kregulator_interome$NCBI.gene.ID.1, protein_distance)
closest_dis(AED_targets_interome$ENTREZID, kregulator_interome$NCBI.gene.ID.1, protein_distance)
...
dist_pval = sum(aver_closd_AEDTargets_ranRegulators < aver_closd_AEDTargets_Regulators)/1000